...
async function main() {
  ...
  server.post("/", async (request, reply) => {
    ...
    let errorMessage;
    if (!type) {
      errorMessage = "please choose one type of searching";
    }

    switch (type) {
      case "query":
        if (!query) {
          errorMessage = "please enter query representing location";
        }
        break;

      case "structured":
        if (!addressCountry || !addressLocality) {
          errorMessage = "please enter country code and locality";
        }
    }
    ...
    if (!errorMessage) {
      if (type === "query") {
        ({ success, message, log } = await getCoordinates({ query }, service));
      } else if (type === "structured") {
        ({ success, message, log } = await getCoordinates(
          {
            streetAddress,
            addressLocality,
            addressDistrict,
  ...
  // https://github.com/superfaceai/geocode-example
